...
d2 <- subset(gapminder, country %in% d1)  #%in% is a function which just means 'in'

head(d2)

ggplot(d2)

ggplot(d2, aes(x = year, y = lifeExp, color = country)) # tells R to color & group by 'country'

#add point and line by 'country'
ggplot(d2, aes(x = year, y = lifeExp, color = country)) + geom_line() + geom_point() 

#alter line width
ggplot(d2, aes(x = year, y = lifeExp, color = country)) + geom_line(lwd=1) + geom_point()

#geom_path() is often equivalent to geom_line() - there are some differences (not here though)
ggplot(d2, aes(x = year, y = lifeExp, color = country)) + geom_path() + geom_point()

p <- ggplot(d2, aes(x = year, y = lifeExp, color = country)) + geom_line() + geom_point()
...
## Just to give you an example of customization

# here we add text to the end points of each line
library(ggrepel)
...
medianlife <- gapminder %>% group_by(year,continent) %>% summarise(median=median(lifeExp))
