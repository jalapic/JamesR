...
ggplot(patricia, aes(x=year, y=n)) + 
  geom_bar(stat='identity', color='black', lwd=.5, fill='gray33',width=.9)
...
Batting_all <- rbind(Batting_early, Batting_late) #rbind joins two datasets by stacking them
...
ggplot(Batting_all, aes(x=avg, fill=period)) +  
  geom_histogram(position="identity", alpha=.7, binwidth=.005) + 
  facet_wrap(~period) +
  geom_vline(aes(xintercept=0.3), color="black", linetype="dashed", size=1)
